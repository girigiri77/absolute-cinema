        ...
        >
          ...
          >
            ...
            <div className="relative aspect-video w-full flex-shrink-0 overflow-hidden">
              {movie.trailerUrl ? (
                <iframe
                  src={toEmbed(movie.trailerUrl) + "?autoplay=1&mute=1&controls=1&rel=0"}
                  title={movie.title}
                ...
                <img src={movie.backdrop || movie.poster} className="h-full w-full object-cover" alt="" />
              )}
            ...
            <div className="grid flex-shrink-0 gap-4 p-4 pb-10 sm:gap-6 sm:p-6 sm:pb-12 md:grid-cols-[160px_1fr] md:p-8 md:pb-12">
              <div className="hidden md:block">
                <div className="overflow-hidden rounded-xl border border-white/10 shadow-[0_20px_50px_-20px_rgba(168,85,247,0.5)]">
                  <img src={movie.poster} alt={movie.title} className="aspect-poster w-full object-cover" />
                </div>
              </div>
              <div className="min-w-0">
                <h2 className="text-2xl font-bold text-white md:text-3xl">{movie.title}</h2>
                <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-white/65">
                  <span className="flex items-center gap-1">
                    <Star className="h-4 w-4 fill-amber-400 stroke-amber-400" />
                    {movie.rating.toFixed(1)} <span className="text-white/40">IMDb</span>
                  </span>
                  <span className="flex items-center gap-1"><Calendar className="h-4 w-4" />{movie.releaseDate || movie.year}</span>
                  {movie.runtime ? (
                    <span className="flex items-center gap-1"><Clock className="h-4 w-4" />{movie.runtime}m</span>
                  ) : null}
                  <span className="rounded border border-white/15 px-1.5 py-0.5 text-xs">{movie.type}</span>
                  {movie.language && (
                    <span className="rounded border border-white/15 px-1.5 py-0.5 text-xs">{movie.language}</span>
                  )}
                </div>
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {movie.genres.map(g => (
                    <span key={g} className="rounded-full border border-white/15 bg-white/5 px-2.5 py-1 text-xs text-white/80">
                      {g}
                    </span>
                  ))}
                </div>
                ...
                {movie.moods.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {movie.moods.map(mid => {
                      ...
                      if (!m) return null;
                      return (
                        <span key={mid} className={"rounded-full bg-gradient-to-r px-2.5 py-1 text-xs font-medium text-white shadow-[0_0_15px_rgba(168,85,247,0.4)] " + m.color}>
                          {m.emoji} {m.label}
                        </span>
                      );
                    })}
                ...
                {movie.platforms.length > 0 && (
                  <div className="mt-5">
                    <div className="text-xs uppercase tracking-wider text-white/40">Streaming on</div>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {movie.platforms.map(p => (
                        <span key={p} className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold text-white">
                          {p}
                        </span>
                      ))}
                    </div>
